...
dt = 1e-8  # увеличенный временной шаг для ускорения, с
...
fig, ax = plt.subplots()
x = np.linspace(0, H, N)
line, = ax.plot(x, rho_history_last[0], 'r', lw=2)
ax.set_xlabel('x, м')
ax.set_ylabel('ρ, Кл/м³')
ax.set_title('Анимация плотности заряда')
ax.set_ylim(-1e-5, 10)

def update(frame):
    line.set_ydata(rho_history_last[frame])
    return line,

ani = FuncAnimation(fig, update, frames=len(rho_history_last), interval=1, blit=True, repeat=False)
ani.pause()
